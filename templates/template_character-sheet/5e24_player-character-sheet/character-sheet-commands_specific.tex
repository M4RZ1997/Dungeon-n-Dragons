...
\newcommand{\renderoldspellsheet}{%
	...
	\ifthenelse{\equal{\BACKEND}{TikZ}}{%
		\begin{pageTikZ}
			...
			\input{\PATH templates/template_character-sheet/pre_5e24_player-character-sheet/spell-sheet/tikz/player-input.tex}
		\end{pageTikZ}
	}{%
		\ifthenelse{\equal{\BACKEND}{PostScript}}{%
			\makebox[\paperwidth][s]{
				...
				{
					\input{\PATH templates/template_character-sheet/pre_5e24_player-character-sheet/spell-sheet/postscript/header.tex}
					\input{\PATH templates/template_character-sheet/pre_5e24_player-character-sheet/spell-sheet/postscript/outlines.tex}
					\input{\PATH templates/template_character-sheet/pre_5e24_player-character-sheet/spell-sheet/postscript/spell-level-headers.tex}
					\input{\PATH templates/template_character-sheet/pre_5e24_player-character-sheet/spell-sheet/postscript/spell-slots.tex}
					
					\input{\PATH templates/template_character-sheet/pre_5e24_player-character-sheet/spell-sheet/postscript/template-text.tex}
					\input{\PATH templates/template_character-sheet/pre_5e24_player-character-sheet/spell-sheet/postscript/player-input.tex}
				}
				\end{pspicture}
			}
...
%	}{%
%		\ifthenelse{\equal{\BACKEND}{PostScript}}{%
%			\PostScriptCustomLabel{#1}%
%		}{\PackageWarning{Display Commands}{Backend "\BACKEND " unknown.}}%
%	}%
%}%

\ExplSyntaxOn
\NewDocumentCommand{\CustomItemLabel}{ m }{%
	\str_case:VnF { \BACKEND }
	{%
		{PostScript}{\PostScriptCustomLabel{#1}}
		{TikZ}{\TikZCustomLabel{#1}}
	}%
	{\PackageWarning{Display Commands}{Backend "\BACKEND " unknown.}}
}%


\NewDocumentCommand{\PostScriptCustomLabel}{ m }{%
	\str_case:nnF { #1 }
	{%
		{A}{%
			\rput(0,5){%
...
\NewDocumentCommand{\TikZCustomLabel}{ m }{%
	\str_case:nnF { #1 }
	{%
		{A}{%
			\begin{tikzpicture}%
		...
		{B}{%
			\begin{tikzpicture}%
				\fill[black] (0,0) ++(0,-3) -- ++(3,6) -- ++(-6,0) -- cycle;%
			\end{tikzpicture}%
		}%
		{R}{%
			\begin{tikzpicture}%
				\fill[black] (0,0) ++(0,-3) -- ++(3,3) -- ++(-3,3) -- ++(-3,-3) -- cycle;%
			\end{tikzpicture}%
		}%
		{L}{%
			\begin{tikzpicture}%
				\fill[black] (0,0) ++(3,-3) -- ++(0,6) -- ++(-6,0) -- ++(0,-6) -- cycle;%
			\end{tikzpicture}%
		}%
	}%
	{\textbullet}%
...
\makeatletter
\newcommand{\ClassFeaturesSection}[1]{%
	\g@addto@macro\ClassFeatures{\item[]\hspace*{-\leftmargin}\textbf{#1}}
}%
\newcommand{\AddClassFeature}[2]{%
	\g@addto@macro\ClassFeatures{\renewcommand{\labelitemi}{\textbf{\CustomItemLabel{#1}}}\item #2}
}%
\makeatother
...
\newcommand{\AddSpeciesFeature}[2]{%
	\g@addto@macro\SpeciesFeatures{\renewcommand{\labelitemi}{\textbf{\CustomItemLabel{#1}}}\item #2}
}%
\makeatother
...
\ExplSyntaxOn
\NewDocumentCommand{\PostScriptCastingTimeSymbol}{ m }{%
	\str_case:nnF { #1 }
	{%
		{Bonus~Action}{%
			\rput(0,0){%
...
\NewDocumentCommand{\TikZCastingTimeSymbol}{ m }{%
	\str_case:nnF { #1 }
	{%
		{Bonus~Action}{%
			\begin{tikzpicture}%
				\fill[black] (0,0) ++(0,-5) -- ++(5,10) -- ++(-10,0) -- cycle;
			\end{tikzpicture}%
		}%
		{Reaction}{%
			\begin{tikzpicture}%
				\fill[black] (0,0) ++(0,-5) -- ++(5,5) -- ++(-5,5) -- ++(-5,-5) -- cycle;
			\end{tikzpicture}%
		}%
	}{#1}%
}%
...
\newcommand{\setCastingTime}[2]{%
	\ifthenelse{\equal{\BACKEND}{TikZ}}{%
		\expandafter\renewcommand\csname Spell#2CastingTimeValue\endcsname{\TikZCastingTimeSymbol{#1}}%
	}{%
		\ifthenelse{\equal{\BACKEND}{PostScript}}{%
			\expandafter\renewcommand\csname Spell#2CastingTimeValue\endcsname{\PostScriptCastingTimeSymbol{#1}}%
		}{\PackageWarning{Display Commands}{Backend "\BACKEND " unknown.}}%
	}%
}%
...
\NewDocumentCommand{\IndicationStar}{ mm }
{%
	\filldraw[line width=1pt, color=black, fill=#2] #1 ++(0,-4) -- ++(-1,0) arc[start angle=0, end angle=90, radius=3] -- ++(0,2) arc[start angle=-90, end angle=0, radius=3] -- ++(2,0) arc[start angle=-180, end angle=-90, radius=3] -- ++(0,-2) arc[start angle=90, end angle=180, radius=3] -- cycle;%
}%

\NewDocumentCommand{\ProficiencyMarker}{ mmm }
{%
	\filldraw[line width=0.3pt, color=black, fill=#2] #1 circle (3);
	\fill[#3] #1 ++(1.5,0) arc[start angle=0, end angle=180, radius=1.5] .. controls ++(0,-0.75) and ++(-0.75,0) .. ++(1.5,0) .. controls ++(0.75,0) and ++(0,-0.75) .. ++(1.5,0) -- cycle;
}%