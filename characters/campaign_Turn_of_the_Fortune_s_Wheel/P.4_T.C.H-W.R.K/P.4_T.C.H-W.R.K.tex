...
% Multiclass Levels for this Character
\def\ArtificerLevel{5}
\def\BardLevel{2}
\def\DruidLevel{6}
\def\RangerLevel{3}
\def\SorcererLevel{1}
...
\CharacterAppearance{vertical} % (vertical/horizontal/none)
	...
	{2} % Picture Y-Shift (absolute value)
...
\AlliesAndOrganizations{
	...
	Founded roughly four centuries ago by the visionary artificer G.E. Petto, the Foundry began as a modest workshop dedicated to crafting mechanical servants. Eventually, the reliability and ingenuity of Petto's designs quickly drove overwhelming demand, forcing rapid expansion from a personal workspace into a full-scale industrial complex. Over time, the Foundry evolved beyond\linebreak
}{
	utility constructs. Through iterative innovation and increasingly sophisticated arcane integration, it began producing specialized automatons for reconnaissance, infiltration, and eventually combat.
}
\Characterbackground{
...
% --------------------------------------------------------------------------------------------------------------- %
\Equipment{
	\textbf{Scroll of Control Weather}, \textbf{Scroll of Earthquake},  \textbf{Eversmoking Bottle}\\
...
\begin{tikzpicture}[remember picture, overlay]
	\node[opacity=1,inner sep=0pt, xshift=-3cm, yshift=3.5cm] at (current page.east){\includegraphics[width=0.175\paperwidth, keepaspectratio]{images/Steel_Defender.png}};
\end{tikzpicture}
\vfill\eject
...
\end{DndMonster}
\begin{tikzpicture}[remember picture, overlay]
	\node[opacity=1,inner sep=0pt, xshift=-2cm, yshift=8.5cm] at (current page.east){\includegraphics[width=0.175\paperwidth, keepaspectratio]{images/Homunculus_Servant.png}};
\end{tikzpicture}
...
\begin{DndMonster}[width=0.5\textwidth]{Ape}
	...
	\DndMonsterBasics[
		...
		hit-points	= {\MaxHitPointsValue\ + \DruidLevel~temp HP},
...
%		damage-immunities		= {},
%		condition-immunities	= {},
		senses					= {Blindsight 30ft., Passive Perception 24},
		languages				= -,
		challenge				= 1/2,
		proficiency-bonus		= \ProficiencyValue,
	]
	
	\DndMonsterAction{Camouflage}
...
\begin{tikzpicture}[overlay, remember picture, inner sep=0pt, outer sep=0pt, path fading=fade down]%
	\node[anchor=south west] at (current page.south west) {%
		\begin{minipage}{\paperwidth}%%
        	\includegraphics[width=\paperwidth]{images/Ape_Jaculi.png}%
      	\end{minipage}%
	};%
\end{tikzpicture}%
\clearpage
...
%		or-dmg			= ,
%		or-dmg-when		= ,
		extra			= {. If the target is a Medium or smaller creature, it has the Grappled condition (escape DC 16). While Grappled, the target has the Restrained condition},
	]
\end{DndMonster}
\begin{tikzpicture}[overlay, remember picture, inner sep=0pt, outer sep=0pt, path fading=fade down]%
	\node[anchor=south west] at (current page.south west) {%
		\begin{minipage}{\paperwidth}%%
        	\includegraphics[width=\paperwidth]{images/Black-Bear_Crocodile.png}%
      	\end{minipage}%
	};%
\end{tikzpicture}%
\clearpage
\subsection*{Giant Sea Eel}
\begin{DndMonster}[width=0.5\textwidth]{Giant Sea Eel}
	\DndMonsterType{Large Beast, Unaligned}
...
\begin{tikzpicture}[overlay, remember picture, inner sep=0pt, outer sep=0pt, path fading=fade down]%
	\node[anchor=south west] at (current page.south west) {%
		\begin{minipage}{\paperwidth}%%
        	\includegraphics[width=\paperwidth]{images/Reef-Shark_Giant-Sea-Eel.png}%
      	\end{minipage}%
	};%
\end{tikzpicture}%
