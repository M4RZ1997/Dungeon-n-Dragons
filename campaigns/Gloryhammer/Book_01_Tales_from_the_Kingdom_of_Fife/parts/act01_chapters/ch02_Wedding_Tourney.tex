...
\vspace*{-2.1\fontdimen6\font}\hfill\\\begin{tabular}{@{}p{0.21\textwidth}p{0.2325\textwidth}@{}}
	\begin{tabular}[t]{@{}p{\linewidth}@{}}
		{\entryfont system for the Wedding Tourney is designed to reward consistent performance across the Archery, Long Throw, and Log Chopping competitions. Competitors earn points based on their\linebreak}
	\end{tabular}
	&
	\vspace*{-2.3em}\begin{tabular}[t]{@{}p{\linewidth}@{}}
		\begin{DndTable}[header=Point Scoring System]{lXr}
...
\subsection*{Bullseye's Glory - Mechanics}
...
\subsection*{Timber Trial - Mechanics}
...
\subsection*{The Grand Joust - Mechanics}
...
\begin{itemize}
	...
	\item \textbf{Success:} The rider gains a +1 bonus to their first Ride Check (Constitution Saving Throw) during the joust.
	\item \textbf{Critical Success (Natural 20):} The rider gains a +2 bonus to their first Ride Check and an additional +1 bonus to a potential subsequent Ride Check.
	\item \textbf{Critical Failure (Natural 1):} The rider suffers a -2 penalty to their first Ride Check and a -1 penalty to all subsequent Ride Checks for the duration of the competition.
\end{itemize}}
\begingroup
	\DndSetThemeColor[PhbTan]
	\begin{DndComment}{DM Annotation: Ride Check (Rules Clarification)}
		In this context a \textit{Ride Check} is equivalent to a Constitution Saving Throw.
	\end{DndComment}
\endgroup
...
\paragraph*{Setup Rules} Contestants use specially crafted tourney lances provided by the organizers. These lances deal 1d4 Piercing (non-lethal) damage. If a rider deals more than 8 damage in a single round, the lance shatters spectacularly, earning the rider a +2 bonus to their next attack roll as the crowd cheers their impressive display of skill and power.

The use of magic during the joust is strictly forbidden and results in immediate disqualification.

\paragraph*{Gameplay} Each pass between the two contestants is resolved through simultaneous strikes. Riders may choose one of the following attack strategies for their turn:
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item If the attack lands, the opposing rider must make a Ride Check with a DC of 15 + damage dealt to stay mounted
	\end{itemize}
	\item \textbf{Crouch Defensively.}
	\begin{itemize}
		\item Attack Roll Penalty: -4 to attack roll
		\item The rider gains a +4 bonus to their Ride Check if struck
	\end{itemize}
\end{itemize}

\noindent When struck by an opponent's lance, a rider must make a Ride Check (DC 5 + damage dealt) to remain in the saddle. Any rider reduced to 0 HP will automatically fail this check.
...
\paragraph*{End Condition} If both riders remain mounted after a pass, the joust continues into another round. After each completed pass (round), the DC for all subsequent Ride Checks increases by +2 to represent the mounting tension and fatigue of the contest. The joust ends when only one rider remains mounted. That rider is declared the winner.
...
\subsection*{Supporting the Champion}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Success.}\\
		The champion gets a +2 bonus on either the next attack roll or the next Ride Check
		...
		The next attack roll and a possible Ride Check are made with Disadvantage
	\end{itemize}
\end{itemize}
\vfill\eject
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Failure.}\\
		The crowd's disapproval leads to the champion being distracted, having disadvantage on the next attack and potential Ride Check
		\item \textbf{Critical Failure.}\\
...
\begin{itemize}
	\item The champion can re-roll a failed Ride Check or attack roll during the joust.
\end{itemize}
}
