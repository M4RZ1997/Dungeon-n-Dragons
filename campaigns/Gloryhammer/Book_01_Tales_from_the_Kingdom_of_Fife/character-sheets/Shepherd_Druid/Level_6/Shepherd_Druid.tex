...
\ToolsProficiency{Carpenter's Tools, Healer's Kit, Herbalism Kit, Pan Flute}
% --------------------------------------------------------------------------------------------------------------- %
...
%%	#7 = Notes
\addWeaponStatistic{Dagger of Venom}{LunaFinn}{DEX}{P}{1}{1d4 +\intcalcAdd{1}{\calculateModifier{\DexterityScoreValue}} p}{Finesse, Light, Thrown (20/60 ft)}
\addWeaponStatistic{Quarterstaff}{LunaFinn}{STR}{P}{0}{1d6 \intcalcAdd{0}{\calculateModifier{\StrengthScoreValue}} b}{Versatile (1d8)}
\addWeaponStatistic{Sickle}{LunaFinn}{STR}{P}{0}{1d4 \intcalcAdd{0}{\calculateModifier{\StrengthScoreValue}} s}{Light}
% UNARMED STRIKE
\addWeaponStatistic{Unarmed Strike}{LunaFinn}{STR}{P}{0}{1d6 \intcalcAdd{0}{\calculateModifier{\StrengthScoreValue}} b}{}
% DAMAGE CANTRIPS
...
\AddClassFeature{L}{\textbf{Wild Shape}\\3 per Long Rest}
...
\AddClassFeature{E}{\textbf{Mighty Summoner}\\Summoned Creature:\begin{itemize}
	\item Increased Hit Points
	\item Attacks are magical
\end{itemize}}
% SPECIES TRAITS
...
\AddFeat{\textbf{Tough}}
\AddFeat{\textbf{Boon of the Great Eagle}\begin{itemize}
	\item[\CustomItemLabel{L}] Once per LR: Reduce incoming damage to you or an ally within 10 feet by 2d8 + \intcalcAdd{0}{\ProficiencyValue}
	\item[\CustomItemLabel{B}] Expend 2 Wild Shape uses to Wild-Shape into a Great Eagle
\end{itemize}}
% --------------------------------------------------------------------------------------------------------------- %
...
% --------------------------------------------------------------------------------------------------------------- %
\Equipment{
	\textbf{Elven Chain}, \textbf{Dagger of Venom}, Quarterstaff, Sickles (2), Wooden Shield\\
	\textbf{Golden Feather (Druidic Focus)}\\
	Gilded Acorn (200 GP), Gilded Flower (300 GP)\\
	Carpenter's Kit, Healer's Kit, Herbalism Kit\\
...
\subsection*{Boon of the Great Eagles}
...
You can expend 2 Wild Shape Uses to become a Great Eagle using the same rules as for "Wild Shape".
...
\subparagraph*{Armor Class} Until you leave the form, your AC equals 13 plus your Wisdom modifier if that total is higher than the Beast's AC. \textbf{(AC \intcalcAdd{13}{\calculateModifier{\WisdomScoreValue}})}
\subparagraph*{Temporary Hit Points} You gain a number of Temporary Hit Points equal to three times your Druid level. \textbf{(\intcalcMul{3}{\LevelValue} Temporary HP)}
...
\begin{itemize}
	\item The creature appears with more hit points than normal: 2 extra hit points per Hit Die it has.
	\item The damage from its natural weapons is considered magical for the purpose of overcoming immunity and resistance to nonmagical attacks and damage.
\end{itemize}
...
\begin{DndMonster}[width=0.5\textwidth]{Bestial Spirit}
	...
	\DndMonsterBasics[
		...
		hit-points	= {28 (Air only) or 38 (Land and Water only) + 7 for each spell level above 2nd (4 + Spell Level d6)},
...
\begin{itemize}
	\item It has an Armor Class of 17 if its AC is lower than that.
	\item It has a +5 bonus on damage rolls with Strength based attacks.
	\item It can make a Bite attack as a Bonus Action. The Bite deals 1d6 + its Strength modifier Slashing damage.
	\item It has Immunity to the Frightened condition.
	\item It can't cast spells or maintain Concentration.
...
\subparagraph*{Using a Higher-Level Spell Slot} The damage increases by 1d10 for each spell slot level above 3.
...
\begin{DndMonster}[width=0.5\textwidth]{Fey Spirit}
	...
	\DndMonsterBasics[
		...
		hit-points	= {42 + 14 for each spell level above 3rd (6 + 2 * Spell Level d6)},
	...
	\DndMonsterAttack[
		...
		%range			= 20/60,
		targets			= one target,
		dmg				= {\DndDice{2d6 + 3} + the spell's level},
		dmg-type		= Force,
		%plus-dmg		= ,
		%plus-dmg-type	= ,
		%or-dmg			= ,
...
%		description = {},
		...
		targets = {one creature the spirit can see within 10 feet of itself},
...
\\
\underline{\textit{Warhammer (Versatile):}}\\
1d20 + STR-Modifier + Proficiency Modifier\\
...
{\entryfont You gain a +1 bonus to Armor Class while you wear this armor. You are considered trained with this armor even if you lack training with Medium or Heavy armor.}

\subsection*{Dagger of Venom}
\textit{Weapon (Dagger), Rare}\\
{\entryfont You gain a +1 bonus to attack rolls and damage rolls made with this magic weapon.

You can take a Bonus Action to magically coat the blade with poison. The poison remains for 1 minute or until an attack using this weapon hits a creature. That creature must succeed on a DC 15 Constitution saving throw or take \DndDice{2d10} Poison damage and have the Poisoned condition for 1 minute. The weapon can't be used this way again until the next dawn.}
...
\begin{tikzpicture}[overlay, remember picture, inner sep=0pt, outer sep=0pt, path fading=fade down]%
	\node[anchor=south west] at (current page.south west) {%
		\begin{minipage}{\paperwidth}%%
        	\centering\includegraphics[width=\paperwidth]{Wolf_Spider_Black_Bear_background}%
      	\end{minipage}%
	};%
\end{tikzpicture}%
...
\begin{tikzpicture}[overlay, remember picture, inner sep=0pt, outer sep=0pt, path fading=fade down]%
	\node[anchor=south west] at (current page.south west) {%
		\begin{minipage}{\paperwidth}%%
        	\centering\includegraphics[width=\paperwidth]{Ape_Warhorse_background}%
      	\end{minipage}%
	};%
\end{tikzpicture}%
\vspace*{-2.7\fontdimen6\font}\hfill\\
...
\begin{tikzpicture}[overlay, remember picture, inner sep=0pt, outer sep=0pt, path fading=fade down]%
	\node[anchor=south west] at (current page.south west) {%
		\begin{minipage}{\paperwidth}%%
        	\centering\includegraphics[width=\paperwidth]{Sea_Horse_Shark_background}%
      	\end{minipage}%
	};%
\end{tikzpicture}%
\vspace*{-2.7\fontdimen6\font}\hfill\\
...
\section*{Giant Eagle}
\begin{tikzpicture}[overlay, remember picture, inner sep=0pt, outer sep=0pt, path fading=fade down]%
	\node[anchor=south west, yshift=-80pt] at (current page.south west) {%
		\begin{minipage}{\paperwidth}%%
        	\centering\includegraphics[width=\paperwidth]{Great_Eagle_background}%
      	\end{minipage}%
	};%
\end{tikzpicture}%
\vspace*{-2.7\fontdimen6\font}\hfill\\
\begin{DndMonster}[width=0.5\textwidth]{Giant Eagle}
	\DndMonsterType{Large Beast}
